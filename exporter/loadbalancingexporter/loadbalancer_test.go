...
func TestNewLoadBalancerNoResolver(t *testing.T) {
	...
	// test
	p, err := newLoadBalancer(ts.Logger, cfg, nil, tb, ts.TelemetrySettings)
...
func TestOnBackendChanges(t *testing.T) {
	...
	p, err := newLoadBalancer(ts.Logger, cfg, componentFactory, tb, ts.TelemetrySettings)
	require.NotNil(t, p)
	require.NoError(t, err)
...
func TestAddMissingExporters(t *testing.T) {
	...
	p, err := newLoadBalancer(ts.Logger, cfg, fn, tb, ts.TelemetrySettings)
	require.NotNil(t, p)
	require.NoError(t, err)
